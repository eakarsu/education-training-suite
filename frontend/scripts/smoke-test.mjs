...
const adminCookie = await login('admin@education-training.local', 'admin123');
const managerCookie = await login('manager@education-training.local', 'manager123');
const analystCookie = await login('analyst@education-training.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/learners', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/learners', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/learners/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/learners/approve`, {
  method: 'POST',
  headers: {
